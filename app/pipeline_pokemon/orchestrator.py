"""
Pokémon Cyberpunk Evolution pipeline orchestrator.
...
from app.schemas.pokemon_schema import PokemonRequest
...
_WORK_BASE = Path("temp/pokemon_jobs")


def create_job(req: PokemonRequest) -> WhatIfJob:
    job_id = uuid.uuid4().hex[:12]
    work_dir = _WORK_BASE / f"pk_{datetime.now().strftime('%Y%m%d')}_{job_id}"
    work_dir.mkdir(parents=True, exist_ok=True)

    job = WhatIfJob(
        job_id=job_id,
        topic=req.pokemon_name,
        model=req.model,
        voice_model=req.voice_model,
        topic_type="pokemon",
    )
    _JOBS[job_id] = job
    logger.info("Created Pokémon job %s for pokemon=%r", job_id, req.pokemon_name)
    return job
...
def _work_dir(job_id: str) -> Path:
    matches = list(_WORK_BASE.glob(f"pk_*_{job_id}"))
    if matches:
        return matches[0]
    d = _WORK_BASE / f"pk_{datetime.now().strftime('%Y%m%d')}_{job_id}"
...
def cleanup_old_work_dirs(max_age_hours: int = 24) -> None:
    ...
    for d in _WORK_BASE.iterdir():
        if d.is_dir() and d.stat().st_mtime < cutoff:
            ...
            logger.info("Cleaned up stale pokémon work dir: %s", d)


async def run_pipeline(job_id: str) -> None:
    from app.pipeline_pokemon import stage0_brain
    ...
    try:
        # ── Stage 0: Pokémon Evolution Brain ────────────────────────────────
        await _push(job, f"⚡ Generating Cyberpunk Evolution for '{job.topic}'...", "brain", 5)
        job.brain_output = await stage0_brain.run(job.topic, job.voice_model, language="en")
        await _push(job, f"✅ Evolution ready. Vibe: {job.brain_output.vibe}", "brain", 15)

        # ── Stage 1 + 2: Veo clips & TTS in parallel ────────────────────────
        await _push(job, "🎬 Generating evolution clips & voiceover in parallel...", "media_gen", 20)
        ...
        # ── Stage 3: Stitch ──────────────────────────────────────────────────
        await _push(job, "✂️ Stitching evolution clips...", "stitch", 62)
        ...
        export_path = export_dir / f"pokemon_{job_id}.mp4"
        ...
        logger.info("[%s] Cleaned up pokémon work dir %s", job_id, work_dir)

    except Exception as exc:
        job.status = WhatIfStatus.failed
        job.error = str(exc)
        logger.exception("[%s] Pokémon pipeline failed: %s", job_id, exc)
